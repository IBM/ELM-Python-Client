...
# Query back the newly created Test Case to get its URL
...
if len(tcs) == 0:
    raise Exception( "Newly created Test Case not found by query!" )
if len(tcs) > 1:
    raise Exception( f"More than one Test Case found with title '{tc_title}' !!!???" )

tc_url = list(tcs.keys())[0]
print(f"Found new Test Case URL: {tc_url}")
