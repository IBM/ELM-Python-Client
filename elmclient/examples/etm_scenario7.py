...
#SCENARIO 7
#
# Query all Test Scripts modified since 2026-01-01
#               -> For each script, GET the full resource and list its steps:
#                  - Step index (rqm_qm:index) and title
#                  - Expected result
#                  - validatesRequirement links (title + target URL)
...
# OSLC query to find all the test scripts modified after 2026-01-01, returns dcterms:title, rqm_qm:shortIdentifier
...
for count, ts_url in enumerate(tss, start=1):
    ...
    # GET the full Test Script resource to retrieve the list of step URLs
    ...
    # fetch all steps and sort them by rqm_qm:index (the authoritative step order)
    ...
    for stepObject in steps:
        ...
        #display validatesRequirement links
        if stepObject.links:
